        ...
        <ul class="nav nav-tabs flex-row" style="height: 55px;">
            @if(is_array($collection_tabs) && count($collection_tabs) > 0)
                @foreach($collection_tabs as $collection)
                    <li class="nav-items">
                        <a class="nav-link fst-italic" data-bs-toggle="tab" href="#collection_{{$collection->id}}">{{$collection->name}}
            ...
            <a style="text-decoration: none" href="{{route('workspace.addCollection',['workspace' => $selectedWorkspace->id])}}" class="d-flex justify-content-center align-items-center p-2 add-nav-items">
